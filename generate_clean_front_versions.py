"""
Roblox Shirt Template Generator - 5 Versions (Clean Front)
Front: Only pattern and logos (NO text/number)
Back: OTTERSON 8 text and number
"""
...
def create_version_1_angular(img, draw):
    """Version 1: Angular geometric pattern"""
    random.seed(42)
    for i in range(60):
...
def create_version_2_faceted(img, draw):
    """Version 2: Faceted/low-poly style"""
    random.seed(123)
    for i in range(50):
        x = random.randint(100, 500)
        y = random.randint(100, 350)
...
def create_version_3_polygonal(img, draw):
    """Version 3: Abstract polygonal pattern"""
    random.seed(456)
    for i in range(70):
        x = random.randint(100, 500)
        y = random.randint(100, 350)
...
def create_version_5_dense(img, draw):
    """Version 5: Dense angular pattern"""
    random.seed(321)
    for i in range(90):
        x = random.randint(100, 500)
        y = random.randint(100, 350)
...
def add_common_elements(draw, font_medium, font_large):
    """Add elements - CLEAN FRONT (no text), text only on BACK"""
    ...
    # === FRONT DESIGN (CLEAN - NO TEXT/NUMBER) ===
    ...
    # === BACK DESIGN (TEXT AND NUMBER HERE) ===
    # Back "OTTERSON"
...
print("Generating 5 Roblox shirt templates (CLEAN FRONT)...")
print("=" * 60)
...
for idx, (name, pattern_func) in enumerate(versions, 1):
    ...
    # Add common elements (clean front, text on back only)
    add_common_elements(draw, font_medium, font_large)

    # Save
    filename = f"otterson_8_final_v{idx}.png"
    img.save(filename, "PNG")
    print(f"[{idx}/5] {name}")
    print(f"       Saved as: {filename}")

print("=" * 60)
print("[SUCCESS] All 5 versions created!")
print("\nFormat: CLEAN FRONT (logos only) + BACK (OTTERSON 8)")
print("\nFiles created:")
for i in range(1, 6):
    print(f"  - otterson_8_final_v{i}.png")
