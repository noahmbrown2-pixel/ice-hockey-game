...
const WIDE_RECEIVERS = [
  {
    ...
    years: "1985–2004",
    highlights:
      "3x Super Bowl champion, 13x Pro Bowl, 10x First-Team All-Pro. All-time leader in receptions (1,549), receiving yards (22,895), and receiving TDs (197).",
  },
  {
    name: "Randy Moss",
    team: "Minnesota Vikings / New England Patriots / others",
    years: "1998–2012",
    highlights:
      "2x First-Team All-Pro, 6x Pro Bowl. 982 receptions, 15,292 yards, 156 TDs. NFL record 23 TDs in 2007 with Patriots.",
  },
  {
    name: "Calvin Johnson",
    team: "Detroit Lions",
    years: "2007–2015",
    highlights:
      "3x First-Team All-Pro, 6x Pro Bowl. 731 receptions, 11,619 yards, 83 TDs. Single-season receiving yards record (1,964 in 2012). Retired at 30.",
  },
  {
    name: "Terrell Owens",
    team: "49ers / Eagles / Cowboys / Bills / Bengals",
    years: "1996–2010",
    highlights:
      "5x First-Team All-Pro, 6x Pro Bowl. 1,078 receptions, 15,934 yards, 153 TDs. 3rd all-time in receiving TDs at retirement.",
  },
  {
    ...
    years: "2004–2020",
    highlights:
      "1x First-Team All-Pro, 11x Pro Bowl. 1,432 receptions (2nd all-time), 17,492 yards, 121 TDs. Legendary 2008 playoff run.",
  },
  {
    name: "Don Hutson",
    ...
    years: "1935–1945",
    highlights:
      "3x NFL Champion, 8x First-Team All-Pro, 2x NFL MVP. Invented the modern pass route. Led league in receiving 8 times. 488 receptions, 99 TDs in an era of ground-game dominance.",
  },
  {
    name: "Lance Alworth",
    ...
    years: "1962–1972",
    highlights:
      '7x AFL All-Star, 3x AFL receiving leader. 542 receptions, 10,266 yards, 85 TDs. Known as "Bambi" for his graceful play. First AFL player inducted into Pro Football Hall of Fame.',
  },
  {
    name: "Steve Largent",
    ...
    years: "1976–1989",
    highlights:
      "7x Pro Bowl. Retired as NFL all-time leader in receptions (819), receiving yards (13,089), and TDs (100). 177 consecutive games with a reception.",
  },
  {
    ...
    years: "1996–2008",
    highlights:
      "3x First-Team All-Pro, 8x Pro Bowl. 1,102 receptions, 14,580 yards, 128 TDs. NFL single-season record 143 receptions in 2002. Elite precision route runner with Peyton Manning.",
  },
  {
    name: "Cris Carter",
    team: "Philadelphia Eagles / Minnesota Vikings / Miami Dolphins",
    years: "1987–2002",
    highlights:
      '2x First-Team All-Pro, 8x Pro Bowl. 1,101 receptions, 13,899 yards, 130 TDs. "All he does is catch touchdowns." Master of the sideline catch.',
  },
];
...
async function runOpeningArguments() {
  ...
  const promises = WIDE_RECEIVERS.map((wr) => {
    const system = `You are a passionate NFL analyst and debate advocate assigned to argue that ${wr.name} is the greatest wide receiver of all time. You must make the strongest possible case using statistics, achievements, impact on the game, era context, and intangible qualities. Be persuasive, specific, and bold. Keep your argument focused and under 400 words.`;
    const user = `Make your opening argument for why ${wr.name} (${wr.team}, ${wr.years}) is the GOAT wide receiver.\n\nKey facts: ${wr.highlights}\n\nPresent your case with conviction. Include specific stats, comparisons, and why this player stands above all others.`;

    console.log(`  → Agent for ${wr.name} starting...`);
    return callAgent(system, user).then((text) => {
      console.log(`  ✓ ${wr.name} argument complete`);
      return { player: wr.name, argument: text };
    });
  });
...
async function runRebuttalRound(roundNum, openingArgs, previousRebuttals) {
  ...
  const allArgsSummary = openingArgs
  ...
  const promises = WIDE_RECEIVERS.map((wr) => {
    const otherArgs = openingArgs
      .filter((a) => a.player !== wr.name)
      .map((a) => `**${a.player}**: ${a.argument}`)
      .join("\n\n---\n\n");

    const system = `You are a passionate NFL analyst advocating for ${wr.name} as the GOAT wide receiver. You are now in rebuttal round ${roundNum}. You must defend your player against the strongest competing arguments and poke holes in other candidates' cases. Be sharp, specific, and persuasive. Keep your rebuttal under 350 words.`;

    const user = `Here are the other 9 agents' arguments:\n\n${otherArgs}${prevRebuttalSummary}\n\nNow deliver your rebuttal. Defend ${wr.name} and counter the strongest competing cases. Be specific about why their arguments fall short compared to ${wr.name}'s case.`;

    console.log(`  → ${wr.name} rebuttal round ${roundNum} starting...`);
    return callAgent(system, user).then((text) => {
      console.log(`  ✓ ${wr.name} rebuttal round ${roundNum} complete`);
      return { player: wr.name, round: roundNum, rebuttal: text };
    });
  });
...
async function runJudge(openingArgs, rebuttalRound1, rebuttalRound2) {
  ...
  const system = `You are an impartial, expert NFL historian and judge presiding over a debate to determine the greatest wide receiver of all time. You must evaluate all arguments and rebuttals fairly, considering: statistical dominance, longevity, peak performance, impact on the game, era-adjusted dominance, postseason performance, and how well each advocate made their case in the debate.
...
You MUST respond with valid JSON only — no markdown, no code fences, no extra text. Return this exact structure:
{
  ...
  const user = `Here is the complete debate transcript. Evaluate all arguments and produce your final ranking.
...
Now produce your final 1-10 ranking with detailed reasoning for each player's placement, a GOAT declaration, and overall debate summary.`;
...
<title>WR GOAT Debate Simulator</title>
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .crown { font-size: 4rem; margin-bottom: 16px; display: block; }
  ...
  .rank-1 .rank-badge { background: linear-gradient(135deg, #f1c40f, #f39c12); color: #000; box-shadow: 0 0 20px rgba(241,196,15,0.4); }
  .rank-2 .rank-badge { background: linear-gradient(135deg, #bdc3c7, #95a5a6); color: #000; }
  ...
  /* ── Declaration box ── */
  .declaration {
    background: linear-gradient(135deg, rgba(108,92,231,0.1), rgba(241,196,15,0.1));
...
<div class="hero">
  ...
  <div class="hero-label">The Greatest Wide Receiver of All Time</div>
  <h1 id="hero-name"></h1>
...
<footer>
  WR GOAT Debate Simulator &mdash; Powered by Gemini AI Multi-Agent System
</footer>
...
(function() {
  ...
  document.getElementById('hero-declaration').textContent = judgeResult.goat_declaration;
  ...
  declBox.innerHTML = '<h3>GOAT Declaration</h3><p>' + escHtml(judgeResult.goat_declaration) + '</p>';
  ...
  players.forEach(player => {
    ...
    if (r1) {
      inner += '<div class="rebuttal-entry"><div class="round-label">Round 1 Rebuttal</div><div class="rebuttal-text">' + escHtml(r1.rebuttal) + '</div></div>';
    }
    if (r2) {
      inner += '<div class="rebuttal-entry"><div class="round-label">Round 2 Rebuttal</div><div class="rebuttal-text">' + escHtml(r2.rebuttal) + '</div></div>';
    }
  ...
  items.push({ phase: 'Phase 1', title: 'Opening Arguments Begin', desc: '10 AI agents each advocate for their assigned wide receiver.', end: false });
  openingArgs.forEach(a => {
    items.push({ phase: 'Phase 1', title: a.player + ' presents opening argument', desc: a.argument.slice(0, 120) + '...', end: false });
  });
  ...
  items.push({ phase: 'Phase 3', title: winner.player + ' declared GOAT', desc: judgeResult.goat_declaration.slice(0, 150) + '...', end: true });
...
async function main() {
  console.log("╔══════════════════════════════════════════════╗");
  console.log("║   WR GOAT Debate Simulator — Multi-Agent    ║");
  console.log("╚══════════════════════════════════════════════╝");
  ...
  console.log(`\nGOAT: ${judgeResult.rankings[0].player}`);
